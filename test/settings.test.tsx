...
import { CopyDataToClipboardButton, ImportDataFromClipboardButton } from '@/routes/settings'
...
test("CopyDataToClipboardButton", async () => {
  const writeToClipboard = vi.spyOn(navigator.clipboard, "writeText").mockResolvedValue()
  const { getByText } = await render(<CopyDataToClipboardButton json={dataJson} />)
...
test("ImportDataFromClipboardButton", async () => {
  ...
  const readFromClipboard = vi.spyOn(navigator.clipboard, "readText").mockResolvedValue(dataJson)
