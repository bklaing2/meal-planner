...
test("GetById", async () => {
  const result = await ingredientActions.GetIngredientById(mockIngredients[1].id);
...
test("GetById - throws error when no ingredient found", async () => {
  ...
  await expect(getById).rejects.toThrow(`Ingredient with id ${ingredientId} not found`)
});
...
test("Rename - throws error when name is empty", async () => {
  const newName = "";
  const ingredientId = mockIngredients[0].id;
  const rename = () => ingredientActions.RenameIngredient(ingredientId, newName);
